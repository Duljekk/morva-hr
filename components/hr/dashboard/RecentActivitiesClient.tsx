 ...
 * - Fetches recent activities (announcements and payslips) via server action
...
export default function RecentActivitiesClient({ initialCount = 3 }: RecentActivitiesClientProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const result = await getRecentActivitiesForDashboard(3);
  ...
  return <RecentActivitiesCard activities={activities} loading={loading} error={error} maxItems={3} />;
}
